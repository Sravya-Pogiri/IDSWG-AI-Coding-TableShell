...
def run_table_shell_app():
    ...
    with st.expander("Add a New Custom Table"):
        st.text_input("Enter name for new table category:", key="new_table_name")
        ...
        def add_custom_table_callback():
            name = st.session_state.get("new_table_name", "").strip()
            pasted_content = st.session_state.get("pasted_table_content", "")

            if not name:
                st.warning("Please enter a name for the new table.")
                return
            if name in st.session_state.generated_tables:
                st.error(f"A table named '{name}' already exists.")
                return
            
            if pasted_content:
                df = text_to_dataframe(pasted_content)
                if not df.empty:
        ...
        st.button("➕ Add Custom Table", on_click=add_custom_table_callback)
